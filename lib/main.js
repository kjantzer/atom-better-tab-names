...
module.exports = {
    ...
    activate(state) {
        ...
        this.subscriptions.add(
            atom.commands.add(
                'atom-workspace', {
                    'better-tab-names:toggle': () => this.toggle()
                }
            )
        );
    ...
    addConfigChangeHandler() {
        this.subscriptions.add(atom.config.onDidChange('better-tab-names.equalsNamesEnabled', () => {
            if (!this.active) {
                return;
            }
        ...
        this.subscriptions.add(atom.config.onDidChange('better-tab-names.numberOfFolders', () => {
          for (const key of tabs) {
              key[1].checkTab();
          }
        }));
